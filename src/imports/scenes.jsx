...
// ── icon-box geometry (380 box) ──────────────────────────────────────────────
// Derived directly from uploads/Boarden App Logo.svg (687×769 artwork):
//   b bbox x[44.6,370.5] y[159.9,621.15]; four right-aligned bars, ends square.
//   Mapped so the whole mark sits centered in the 380 box (≈290 wide, ≈208 tall).
// b (Lexend Deca text, weight 500 to match the icon) — ink top≈86, baseline≈294
const B_FONT = 290, B_LEFT = 40, B_TOP = 46;
const LINE_H = 20;
const XR = 335;                            // shared right edge — bars run to the box edge
const LINES = [                            // ragged-left, top bar is the LONGEST (matches logo)
  { x0: 126, x1: XR, cy: 98 },             // longest, near b top
  { x0: 207, x1: XR, cy: 159 },
  { x0: 220, x1: XR, cy: 219 },            // shortest
  { x0: 200, x1: XR, cy: 280 },            // near b bottom
];
...
const T = {
  ...
  thumbA: 2.1, thumbB: 2.9,                  // the b's stem (the “thumb”) bends in, then springs straight
  straightA: 2.2, straightB: 2.9,            // …waves straighten as the thumb settles
  nodA: 2.9, nodB: 3.3,
  wordA: 3.3, wordB: 3.9,
  fadeA: 4.05, fadeB: 4.35,
};

const BEND = 17;                            // peak inward bend of the b's stem (the wiggling thumb), deg
...
function BoardenMark({ t }) {
  ...
  // “thumb wiggle”: the b's vertical stem reads as an upright thumb; it bends
  // inward (skewing the glyph) then springs back straight into the clean icon b.
  const thumbSeg = seg(t, T.thumbA, T.thumbB);
  const bend = BEND * Math.sin(thumbSeg * Math.PI);               // 0 → in → 0, a single wiggle
  const thumbPop = 1 + 0.05 * Math.sin(thumbSeg * Math.PI);        // tiny life as it flicks
  ...
  return React.createElement('div', {
    ...
    // the “b” — real Lexend Deca (weight 500); its stem is the thumb that wiggles,
    // pivoting at the base, then settles into the clean app-icon b.
    React.createElement('div', {
      style: {
        ...
        transform: `skewX(${-bend}deg) scale(${bScale})`, transformOrigin: '64px 236px',
      },
    }, 'b'),
